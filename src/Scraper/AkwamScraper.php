<?php
class AkwamScraper extends BaseScraper {
    public int $priority = 7;

    public function detectFromUrl(string $url): bool {
        return (bool) preg_match('/akwam\.(ws|cx|me)/i', $url);
    }
    ...
    public function parseSearchResults(string $html): array {
        ...
        for ($i = 0; $i < count($matches[0]); $i++) {
            ...
            if ($title && preg_match('/\/(movie|series)\//i', $href)) {
                $results[] = [
                    'title' => $title, 'url' => $this->toAbsolute($href),
                    'poster' => $this->toAbsolute($matches[3][$i]), 'type' => $type, 'source' => 'akwam',
                ];
            }
        }
        return $results;
    ...
    public function getDetails(string $url): ?array {
        $cached = Cache::getMetadata($url, 'akwam');
        ...
        if (preg_match('/<title>(.*?)<\/title>/si', $html, $m)) {
            $title = $this->cleanText(str_replace('| أكوام', '', $m[1]));
        }
        $poster = '';
        ...
        $data = [
            ...
            'type' => $type, 'year' => $year, 'description' => $description, 'source' => 'akwam',
        ];
        Cache::setMetadata($url, 'akwam', $data);
        return $data;
    }

    public function getStreams(string $url): array {
        $cached = Cache::getStreams($url, 'akwam');
            ...
            foreach ($texts as $text) {
                ...
                foreach ($m[1] as $streamUrl) {
                    ...
                    $streams[] = [
                        ...
                        'referer' => $iframeUrl, 'server_name' => 'Akwam',
                    ];
                }
            }
        }
        if (!empty($streams)) Cache::setStreams($url, 'akwam', $streams);
        return $streams;
    }
}
